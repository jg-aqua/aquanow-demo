...
export default function SellOrderSummary({ asset, usdAmount, cryptoAmount, onBack }) {
  const navigate = useNavigate();
  const [success, setSuccess] = useState(false);

  const vat = parseFloat((usdAmount * 0.05).toFixed(2));
  const total = parseFloat((usdAmount - FEES - vat).toFixed(2));

  const rows = [
    { label: 'Quantity', value: `${cryptoAmount.toFixed(6)} ${asset?.symbol}` },
    { label: 'Price', value: formatUSD(asset?.price_usd || 0) },
    { label: 'Order Type', value: 'Sell' },
      ...
      <div>
        ...
        <div className="px-6 pt-14 flex flex-col items-center text-center">
          ...
          <p className="text-muted-foreground text-sm mt-2">
            You sold {cryptoAmount.toFixed(6)} {asset?.symbol} for {formatUSD(usdAmount)}.
          </p>
        </div>
      </div>
    );
